...
def process_madlib(mad_lib):
    ...
    box_length = 4
    idx = 0
    mad_length = len(mad_lib)
    
    while idx < mad_length:
        processed_len = len(processed)
        mad_slice = mad_lib[idx:(idx+box_length)]
        processed = processed + word_transformer(mad_slice)
        #print " so far:    " + processed
        #print " mad slice: " + mad_slice
        if processed_len + 1 == len(processed):
            idx += 1
        else:
            idx += 4
